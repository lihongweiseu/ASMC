...
N = 1:50;
for jj = N
    R1 = corr(x(jj:end,1),Num_resp.Data(1:end-jj+1,1));
    r1(jj) = R1;
%     R4 = corr(x(jj:end,1),Ref_Resp(1:end-jj+1,1));
%     r4(jj) = R4;
end
...
% 2. Normalized root mean square of experimental tracking error
    J2(ii,1) = round(10000*std(x(:,1)-Num_resp.Data(:,1))/std(Num_resp.Data(:,1)))/100;
% 3. Tracking peak error
    J3(ii,1) = round(10000*max(abs(x(:,1)-Num_resp.Data(:,1)))/max(abs(Num_resp.Data(:,1))))/100;
% 4. Displacement normalized root mean square error (1st floor)
    J4(ii,1) = round(10000*(std(x(:,1)-Ref_Resp(:,1))/std(Ref_Resp(:,1))))/100;
% 5. Displacement peak error (1st floor)
    J5(ii,1) = round(10000*max(abs(x(:,1)-Ref_Resp(:,1)))/max(abs(Ref_Resp(:,1))))/100;
